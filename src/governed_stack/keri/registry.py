...
class KeriConsumer:
    ...
    rgy_id: Optional[int] = None  # id() of Regery instance
    purpose: str = ""
...
class FractureReport:
    ...
    fractured: bool
    hby_count: int  # Number of unique Habery instances
    rgy_count: int  # Number of unique Regery instances
    consumers: List[KeriConsumer] = field(default_factory=list)
    hby_groups: Dict[int, List[str]] = field(default_factory=dict)  # hby_id -> [consumer names]
...
def register_keri_consumer(
    ...
    Args:
        name: Unique name for this consumer (e.g., "credential_service")
